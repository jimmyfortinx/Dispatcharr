  ...
  const generateStatusString = (data) => {
    ...
    switch (content.type) {
      ...
      case 'vod_refresh':
        return (
          <StatusBox>
            <Text size="xs" fw={500}>{parseInt(content.progress)}%</Text>
            <Text size="xs" c="dimmed" lineClamp={2} style={{ lineHeight: 1.3 }}>
              {content.phaseMessage}
            </Text>
            <Progress value={content.progress} size="xs" />
            {content.itemProgress && (
              <Text size="xs" c="dimmed">{content.itemProgress}</Text>
            )}
          </StatusBox>
        );
      case 'error':
