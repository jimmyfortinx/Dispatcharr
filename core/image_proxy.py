...
def _scheme_fallback_url(url: str) -> str | None:
    """Return *url* with http/https swapped, or None when it has no swappable scheme."""
...
def _image_candidate_urls(url: str) -> list[str]:
    """The stored URL first, then the same URL under the other scheme.

    Providers routinely store artwork under a scheme their host does not
    actually serve, which surfaces as a transport error rather than a 404.
    """
    candidates = []
    for candidate in (str(url or "").strip(), _scheme_fallback_url(url)):
    ...
    *,
    ...
    for _ in range(IMAGE_FETCH_MAX_REDIRECTS + 1):
        try:
            ...
            _remember_fetch_failure(cache_key, failure_cache, fail_ttl)
...
def serve_local_or_remote_image(
    ...
    *,
    ...
    for candidate_url in _image_candidate_urls(url):
        try:
            ...
            # A refused connection or TLS handshake failure often just means the
            # stored scheme is wrong, so try the other one. Transport errors are
            # never negative-cached: the host may be reachable moments later.
            transport_error = e
            logger.warning(
                "Connection error fetching remote %s %s: %s", log_label, candidate_url, e
            )
            continue
    ...
    logger.warning(
        "Error fetching remote %s %s: %s", log_label, url, transport_error
    )
