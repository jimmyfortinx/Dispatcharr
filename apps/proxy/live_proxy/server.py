...
class ProxyServer:
    ...
    def _handle_stream_switch_event(self, channel_id, data):
        ...
        if new_url == stream_manager.url:
            # update_url() returns False for the same URL; still a success so
            # metadata is refreshed.
            logger.info(
                f"Channel {channel_id} already using requested URL, refreshing metadata only"
            )
            success = True
        else:
        ...
        if success:
            ...
            if self.redis_client:
                try:
                    ...
                    logger.error(
                        f"Error updating switch metadata for channel {channel_id}: {e}",
                        exc_info=True,
                    )
            ...
            # Roll back the URL in metadata to what the manager will actually
            # reconnect to. The non-owner may have pre-written the desired URL;
            # stream_manager.url is the ground truth.
            if self.redis_client:
                metadata_key = RedisKeys.channel_metadata(channel_id)
                self.redis_client.hset(
                    metadata_key, ChannelMetadataField.URL, stream_manager.url
                )
        ...
        try:
            ...
            if self.redis_client:
                ...
                if not url and existing_metadata:
                    url_value = existing_metadata.get('url')
                    if url_value is None:
                        url_value = existing_metadata.get(b'url')
                    if url_value:
                        channel_url = url_value.decode('utf-8') if isinstance(url_value, bytes) else url_value

                    ua_value = existing_metadata.get('user_agent')
                    if ua_value is None:
                        ua_value = existing_metadata.get(b'user_agent')
                    if ua_value:
                        channel_user_agent = ua_value.decode('utf-8') if isinstance(ua_value, bytes) else ua_value

                    input_headers_value = existing_metadata.get(ChannelMetadataField.INPUT_HEADERS)
                    if input_headers_value is None:
                        input_headers_value = existing_metadata.get(ChannelMetadataField.INPUT_HEADERS.encode('utf-8'))
                    if input_headers_value:
                        if isinstance(input_headers_value, bytes):
                            input_headers_value = input_headers_value.decode('utf-8')
                        try:
                            channel_input_headers = json.loads(input_headers_value)
                        except Exception:
                            logger.debug(f"Could not parse input headers for channel {channel_id}")

                # Get stream ID from metadata if not provided
                stream_id_value = existing_metadata.get('stream_id') if existing_metadata else None
                if stream_id_value is None and existing_metadata:
                    stream_id_value = existing_metadata.get(b'stream_id')
                if not channel_stream_id and stream_id_value:
                    try:
                        if isinstance(stream_id_value, bytes):
                            stream_id_value = stream_id_value.decode('utf-8')
                        channel_stream_id = int(stream_id_value)
