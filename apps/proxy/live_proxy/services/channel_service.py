...
class ChannelService:
    ...
    def change_stream_url(
        ...
        if not new_url and target_stream_id:
            ...
            # input_headers stays None for providers that need no extra headers,
            # so it is not a trigger for the lookup.
            if user_agent is None or m3u_profile_id is None:
                stream_info = get_stream_info_for_switch(channel_id, target_stream_id)
                if 'error' in stream_info:
                    # The caller already resolved a URL, so a failed context
                    # lookup is not fatal; keep what we were given.
                    logger.warning(
                        f"Could not backfill switch context for channel {channel_id}: "
                        f"{stream_info['error']}"
                    )
                else:
