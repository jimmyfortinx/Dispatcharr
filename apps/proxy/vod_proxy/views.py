...
def _active_vod_account_filters():
    """Only accounts that are active *and* have VOD enabled may serve playback."""
    return {
...
def _select_vod_stream(
    ...
    for cand, selected_profile in candidate_profiles:
        ...
        # _build_vod_stream_url only understands XC (transformed-credential)
        # accounts. Stalker VOD relies on portal-resolved playback URLs plus
        # per-request auth headers, so route those through the general
        # provider-aware resolver instead.
        input_headers = None
        if cand_account.account_type == M3UAccount.Types.STALKER:
            cand_context = _get_stream_context_for_request(cand, session_id=session_id)
            # Apply the selected M3U profile's URL transform on top of the
            # portal-resolved playback URL, same as XC/live playback.
            final_stream_url = _transform_url(cand_context.get("url"), m3u_profile)
            input_headers = cand_context.get("input_headers")
        else:
    ...
    try:
        ...
        if content_type == 'movie':
            ...
            candidates = list(
                ...
                .filter(**_active_vod_account_filters())
...
_EMPTY_STREAM_CONTEXT = {"url": None, "user_agent": None, "input_headers": None}
    ...
    try:
        ...
        if hasattr(relation, 'get_stream_url'):
            ...
            if url:
                ...
                return {"url": url, "user_agent": None, "input_headers": None}
        ...
        return dict(_EMPTY_STREAM_CONTEXT)
    except Exception as e:
...
def _get_stream_url_from_relation(relation):
    """Backward-compatible URL-only accessor."""
    return _get_stream_context_from_relation(relation).get("url")
...
class VODStreamView:
    ...
    def _delegate(self, view_func, helper_overrides, *args, **kwargs):
        original_helpers = {}
        try:
            for name, value in helper_overrides.items():
                original_helpers[name] = globals()[name]
                globals()[name] = value
    ...
    def get(self, request, content_type, content_id, session_id=None, profile_id=None, user=None):
        return self._delegate(
            stream_vod,
            {
                "_get_content_and_relation": self._get_content_and_relation,
                "_get_m3u_profile": self._get_m3u_profile,
            },
            request,
            content_type,
            content_id,
...
def stream_vod(request, content_type, content_id, session_id=None, profile_id=None, user=None):
    ...
    try:
        ...
        response = connection_manager.stream_content_with_session(
            ...
            input_headers=selected.get("input_headers"),
            user=user,
        )
...
def head_vod(request, content_type, content_id, session_id=None, profile_id=None):
    ...
    try:
        ...
        if selected.get("input_headers"):
            headers.update(selected["input_headers"])
...
def stream_xc_movie(request, username, password, stream_id, extension):
    ...
    try:
        # XC movie catalogs expose the Movie id as the stream ID; older cached
        # client URLs may still carry an M3UMovieRelation id.
        movie_relation = (
