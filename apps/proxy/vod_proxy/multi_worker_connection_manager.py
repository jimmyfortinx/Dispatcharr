...
class MultiWorkerVODConnectionManager:
    ...
    def _build_provider_request_headers(self, m3u_profile, client_user_agent, request, input_headers=None):
        ...
        m3u_user_agent = m3u_profile.m3u_account.get_user_agent_string()
        if m3u_user_agent:
            headers['User-Agent'] = m3u_user_agent
            logger.info(f"Using M3U account user-agent: {m3u_user_agent}")
    ...
    def stream_content_with_session(self, session_id, content_obj, stream_url, m3u_profile,
        ...
        try:
            ...
            if not existing_state:
                ...
                # modified_stream_url/headers were already computed above via
                # _apply_timeshift_parameters/_build_provider_request_headers
                # (the latter also folds in any Stalker input_headers).
