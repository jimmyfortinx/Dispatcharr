...
class M3UAccountProfile(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Auto-sync exp_date from custom_properties for provider-managed accounts.

        XC and Stalker portals report the expiry themselves; for every other
        account type exp_date is set directly and left untouched here.
        ...
        # XC and Stalker portals both report the expiry in the account payload,
        # so exp_date is provider-managed for those and manual for the rest.
        provider_managed_exp_date = bool(self.m3u_account_id) and (
            self.m3u_account.account_type
            in (M3UAccount.Types.XC, M3UAccount.Types.STALKER)
        )
        if provider_managed_exp_date:
            parsed = self._parse_exp_date_from_custom_properties()
            if parsed is not None:
                # Provider reported an exp_date in custom_properties — always sync
        ...
        # else: keep whatever exp_date is already set (manual entry)
        super().save(*args, **kwargs)
