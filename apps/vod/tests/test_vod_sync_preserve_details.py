...
class VODSyncPreserveDetailsTests(TestCase):
    ...
    @patch("core.xtream_codes.Client")
    def test_refresh_skips_when_advanced_detail_is_cached(self, mock_client_cls):
        result = refresh_movie_advanced_data(self.relation.id, force_refresh=False)

        self.assertEqual(result, "Advanced data already cached, skipping.")
        mock_client_cls.assert_not_called()

    @patch("core.xtream_codes.Client")
    def test_refresh_still_skips_cached_detail_after_age_window(self, mock_client_cls):
        """Cached advanced detail is reused regardless of age; only force_refresh refetches."""
    ...
    @patch("core.xtream_codes.Client")
    def test_refresh_runs_when_no_cached_detail(self, mock_client_cls):
        props = dict(self.relation.custom_properties or {})
        props.pop("detailed_info", None)
        props["detailed_fetched"] = False
        self.relation.custom_properties = props
