...
class StreamXcViewTests(SimpleTestCase):
    ...
    def test_m3u8_extension_redirects_to_ts_url(self):
        request = self.factory.get(
            "/live/test-user/test-pass/42.m3u8",
            {"output": "mpegts"},
            HTTP_USER_AGENT="UHF/1.0",
        )
        xc_user = SimpleNamespace(
            username="test-user",
            custom_properties={"xc_password": "test-pass"},
        ...
        with patch(
            ...
            response = stream_xc(request, "test-user", "test-pass", "42.m3u8")

        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response["Location"],
            "/live/test-user/test-pass/42.ts?output=mpegts",
        )
        mock_stream_impl.assert_not_called()
